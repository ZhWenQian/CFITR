...
from model.modules.objectives import compute_irtr_recall,precompute_data, two_stage_retrieval
...
def epoch_wrapup(pl_module):
    ...
    if pl_module.hparams.config["get_recall_metric"] and not pl_module.training:
        compute_irtr_recall(pl_module, phase)

def save_data(pl_module):
    phase = "train" if pl_module.training else "val"

    if pl_module.hparams.config["get_recall_metric"] and not pl_module.training:
        precompute_data(pl_module, phase)

def corase_to_fine_retrieval(pl_module):
    two_stage_retrieval(pl_module)
